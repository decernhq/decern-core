...
type Props = {
  ...
  /** When "team", only highImpact + judge fields are shown. Business+ shows all policies. */
  planId: string;
};
...
export function WorkspacePoliciesForm({ workspaceId, initial, planId }: Props) {
  ...
  const [highImpact, setHighImpact] = useState(initial.high_impact);
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap">
        <label className="flex cursor-pointer items-center gap-2">
          ...
          />
          <span className="text-sm font-medium text-gray-700">{t("highImpact")}</span>
        </label>
        <p className="w-full text-xs text-gray-500 sm:ml-6">{t("highImpactHint")}</p>
      </div>
