...
| **Fair-use cap (server fallback)** | When BYO `llm` is omitted and backend uses `OPEN_AI_API_KEY`, Team is capped at €20/month and Business at €35/month by default **per owner account across all workspaces** (configurable via `JUDGE_FAIR_USE_TEAM_CAP_CENTS` and `JUDGE_FAIR_USE_BUSINESS_CAP_CENTS`). When cap is reached, response is advisory (`allowed: false`, `advisory: true`) and the LLM is not called. |
